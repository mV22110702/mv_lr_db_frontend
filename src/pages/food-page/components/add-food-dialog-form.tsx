...
type AddFoodFormData = CreateFoodDto;
export const useAddFoodFormSchema = (): yup.ObjectSchema<
  AddFoodFormData,
  AnyObject,
  {
    name: undefined;
...
export const AddFoodDialogForm = ({
  setOpenAddFoodDialog,
}: {
  setOpenAddFoodDialog: React.Dispatch<boolean>;
}) => {
  const [create, result] = foodApiSlice.useCreateFoodMutation();
  const onSubmit = (data: AddFoodFormData) => {
    create(data);
  };
  useEffect(() => {
    if (result.isSuccess) {
  ...
  const form = useForm<AddFoodFormData>({
    resolver: yupResolver(useAddFoodFormSchema()),
    defaultValues: {
      name: "",
      amount: 0,
